...
def climber_home(username):
    try:
        ...
        highest_grade_query = text("""
            SELECT 
                c.highest_grade,
                COUNT(a.attempt_id) AS climbs_at_highest
            FROM climber c
            LEFT JOIN attempts a 
                ...
                AND a.route_id IN (
                    SELECT r.route_id
                    FROM route r
                    WHERE r.grade = c.highest_grade
                )
            WHERE c.username = :username
            GROUP BY c.highest_grade;
        """)
        hg_result = g.conn.execute(highest_grade_query, {'username': username}).fetchone()

        stats = {
            'total_routes': (result[0] if result else 0),
            'avg_effort': (result[1] if result else 0),
            'top_wall': (result[2] if result else "N/A"),
            'highest_grade': (hg_result[0] if hg_result else 0),
            'climbs_at_highest': (hg_result[1] if hg_result else 0)
        }
...
@app.route('/session/<username>/<int:session_id>', methods=['GET', 'POST'])
def session_detail(username, session_id):
    ...
    query = text("SELECT notes FROM session WHERE session_id = :session_id")
    note_result = g.conn.execute(query, {'session_id': session_id}).fetchone()
    current_notes = note_result[0] if note_result else ''

    return render_template('session_detail.html',
                           attempts=attempts,
                           stats=stats,
                           session_id=session_id, username=username, current_notes=current_notes)


@app.route('/save_notes/<username>/<session_id>', methods=['POST'])
def save_notes(username, session_id):
    try:
        ...
        with engine.begin() as conn:
            conn.execute(query, {'notes': new_notes, 'session_id': session_id})
        g.conn.commit()
        print(f"✅ Notes saved for session {session_id}: {new_notes}")
        return redirect(f"/session/{username}/{session_id}")
    except Exception as e:
...
def summaries(username):
    ...
    if groupby == 'type':
        ...
        extra_field = ", ROUND(AVG(a.personal_difficulty_rating - r.grade)::numeric, 2) AS avg_effort_diff"
    else:  # default = wall
